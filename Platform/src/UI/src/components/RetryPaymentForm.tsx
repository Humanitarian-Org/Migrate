...
interface RetryPaymentFormProps {
  open: boolean;
  onClose: () => void;
  payment: PaymentProcessingResult | null;
  originalData: any; // Original payment data from the upload
...
const RetryPaymentForm: React.FC<RetryPaymentFormProps> = ({
  ...
  const validateField = (field: keyof PaymentFormData, value: string): string => {
    switch (field) {
      case 'firstName':
  ...
  const validateForm = (): boolean => {
    ...
    Object.keys(formData).forEach(key => {
      const field = key as keyof PaymentFormData;
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Call the Payments domain API directly
      const recordId = payment?.paymentId || payment?.beneficiaryId || '';
      ...
      console.error('Error retrying payment registration:', err);
      setError('Failed to register payment. Please check your connection and try again.');
    } finally {
      setLoading(false);
    }
  };

  if (!open || !payment || !originalData) {
    return null;
  }
      ...
      <DialogTitle>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography variant="h6">
            Retry Payment: {payment.firstName} {payment.lastName}
          </Typography>
        </Box>
      </DialogTitle>
