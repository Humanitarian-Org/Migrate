...
public sealed class CosmosRepository
{
    ...
    // Atomic patch operation to update individual beneficiary record status
        ...
        where T : CosmosItem
    {
        ...
        // Find the array index for the record with matching RecordId
        // This requires a read operation first to find the index
        var document = await ReadAsync<T>(docId, correlationId, ct);
        
        // Assuming Records is an array property - we need to find the index
        // This is a simplified approach - in reality you might need to implement this differently
        // based on your specific document structure
        
        // For now, let's add patch operations that target a specific record by ID
        // We'll need to read the document first to find the correct array index
        var recordsArray = document.Resource.GetType().GetProperty("Records")?.GetValue(document.Resource) as Array;
        
        if (recordsArray != null)
        {
            for (int i = 0; i < recordsArray.Length; i++)
            {
                var record = recordsArray.GetValue(i);
                var recordIdProperty = record?.GetType().GetProperty("RecordId");
                var currentRecordId = recordIdProperty?.GetValue(record) as string;
                
                if (currentRecordId == recordId)
                {
                    // Found the matching record, create patch operations for this index
                    patchOps.Add(PatchOperation.Set($"/Records/{i}/Result/Status", status));
                    patchOps.Add(PatchOperation.Set($"/Records/{i}/Result/ProcessedAt", DateTimeOffset.UtcNow));
                    
                    if (beneficiaryId != null)
                        patchOps.Add(PatchOperation.Set($"/Records/{i}/Result/BeneficiaryId", beneficiaryId));
                    
                    if (errorMessage != null)
                        patchOps.Add(PatchOperation.Set($"/Records/{i}/Result/ErrorMessage", errorMessage));
        ...
        if (!patchOps.Any())
        {
            throw new InvalidOperationException($"Could not find beneficiary record with RecordId: {recordId}");
        }
